...
const REPO_URL = "https://github.com/blue-steeldev/blue-steel-scaffold";
...
async function establishProjectInfo(info: Partial<ProjectInfo>): Promise<ProjectInfo> {
    ...
    while (true) {
        ...
        if (isCancel(model)) {
            log.warn("Didn't like the model selection? Only larger 'visually grounded' models work with BlueSteel");
            log.message("See docs for details: https://docs.blue-steel.run/customizing/llm-configuration");
            process.exit(0);
        }

        if (model === 'docs') {
            log.warn("Only larger 'visually grounded' models work with BlueSteel");
            log.message("See docs for details: https://docs.blue-steel.run/customizing/llm-configuration");
        } else {
            break;
        }
...
export function getMachineId(): string {
    const dir = path.join(os.homedir(), '.blue-steel');
...
program
    .name("create-blue-steel-app")
    .description("Create a new BlueSteel project from a template.")
    ...
    .action(async (projectName) => {
        ...
        intro(`create-blue-steel-app@${VERSION}`);
        ...
        console.log(`◆ Check out our docs: ${blueBright('https://docs.blue-steel.run')}`);
