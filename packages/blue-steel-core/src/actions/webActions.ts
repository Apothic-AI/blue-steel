...
export const cfSolveAction = createAction({
    name: 'browser:cf:solve',
    description: 'Solve a Cloudflare Turnstile / interstitial challenge if present on the page',
    schema: z.object({
        timeout: z.number().optional().describe('Seconds to wait for challenge completion'),
        attempts: z.number().int().optional(),
    }),
    resolver: async ({ input, agent }) => {
        const harness = agent.require(BrowserConnector).getHarness();
        await harness.cfSolve({
            timeout: input.timeout ?? 30,
            attempts: input.attempts ?? 3,
        });
    },
    render: () => `🛡 solve cloudflare challenge`,
});

export const ensureContainerAction = createAction({
    name: 'browser:container:ensure',
    description: 'Ensure a Firefox Multi-Account Container exists',
    schema: z.object({
        name: z.string(),
    ...
    resolver: async ({ input, agent }) => {
        await agent.require(BrowserConnector).getHarness().ensureContainer(
            input.name,
            input.color,
            input.icon
        );
    },
    render: ({ name }) => `▤ ensure container ${name}`,
});

export const openContainerAction = createAction({
    name: 'browser:container:open',
    description: 'Open a URL in a Firefox container tab',
    schema: z.object({
        url: z.string(),
        container: z.string().describe('Container name or firefox-container-N id'),
    }),
    resolver: async ({ input, agent }) => {
        await agent.require(BrowserConnector).getHarness().openInContainer(input.url, input.container);
    },
    render: ({ url, container }) => `▤ open ${url} in ${container}`,
});
