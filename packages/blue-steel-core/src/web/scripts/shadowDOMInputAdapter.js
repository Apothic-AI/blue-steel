...
module.exports = function getShadowDOMInputAdapterScript() {
  return function() {
    if (typeof window.blueSteelShadowDOMAdapter !== 'undefined') {
      console.warn('blueSteelShadowDOMAdapter already defined. Cleaning up old one.');
      if(typeof window.blueSteelShadowDOMAdapter.cleanup === 'function') {
        window.blueSteelShadowDOMAdapter.cleanup();
      }
    }

    window.blueSteelShadowDOMAdapter = {
    ...
    window.blueSteelShadowDOMAdapter.init();
  }.toString();
}
